...
class BookMetaSpider(Spider):
    ...
    sql = 'SELECT * FROM subjects WHERE douban_id NOT IN \
...
-sibling::br]'
    ...
    def parse(self, response):
        if 35000 > len(response.body):
            print("book.meta.response.body:", response.body)
            print("book.meta.response.url", response.url)
        elif 404 == response.status:
            print("book.meta.response.404.url", response.url)
        else:
            meta = BookMeta()
